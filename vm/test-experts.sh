...
run() {
  ...
  python3 moe_run.py $model --packed $packed --draft $draft --threads 4 -n 96 $EXTRA -p "$prompt" > /tmp/$name.out 2> /tmp/$name.err &
  local pid=$!; low_mem $pid & local m1=$!; disk_rate $pid & local m2=$!; wait $pid; echo "exit $?"; wait $m1 $m2
  grep -E "^(plan|helper|quality):" /tmp/$name.out
  grep -oE "Prompt: [0-9.]+ t/s \| Generation: [0-9.]+ t/s" /tmp/$name.out
  grep -E "error|failed|Killed" /tmp/$name.err | head -3
}
# refresh code from Windows and rebuild (moe-stream.h changed)
cp /mnt/c/Users/FSociety/moe-router-study/moe_run.py /root/moe/
cp /mnt/c/Users/FSociety/moe-router-study/llama.cpp/common/moe-stream.h /root/moe/llama.cpp/common/
(cd /root/moe/llama.cpp && cmake --build build -j 4 --target llama-cli >/tmp/build.log 2>&1) || tail -5 /tmp/build.log
...
run "122b-k8"  $M122 models/122b/experts-packed models/Qwen3.5-0.8B-Q4_K_M.gguf "$P3"
EXTRA="--experts 7" run "122b-k7" $M122 models/122b/experts-packed models/Qwen3.5-0.8B-Q4_K_M.gguf "$P3"
EXTRA="--experts 6" run "122b-k6" $M122 models/122b/experts-packed models/Qwen3.5-0.8B-Q4_K_M.gguf "$P3"
