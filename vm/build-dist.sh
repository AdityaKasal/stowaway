...
cp $SRC/moe.py $SRC/repack_experts.py $SRC/pack_dense.py .
...
venv/bin/python -m PyInstaller --onefile --name moe --paths llama.cpp/gguf-py --distpath dist/linux --workpath build-pyi \
...
objdump -T dist/linux/llama-server dist/linux/moe | grep -o 'GLIBC_[0-9.]*' | sort -uV | tail -1
