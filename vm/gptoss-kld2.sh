...
echo "=== second run, no hook  $(date +%T)"; MOE_CACHE_BONUS=0 app/llama-perplexity $ARGS --kl-divergence-base results/gptoss-kld/base.kld --kl-divergence > results/gptoss-kld/base2.txt 2>&1; grep -hE "Mean    KLD|Same top p:" results/gptoss-kld/base2.txt | sed "s/^/  /"
