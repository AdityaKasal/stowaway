...
Run "sp3-none-pk-now"    "3.2" @("--no-warmup") $true
Run "sp3-none-pk-now-cp0" "3.2" @("--no-warmup", "-ctxcp", "0") $true
Run "sp3-d3-pk-now"      "2.6" ($draft + @("--spec-draft-n-max", "3", "--no-warmup")) $true
"`n--- same text?"
$ref = (Get-Content results\spec-none.out.txt -Raw) -replace "\[ Prompt.*", ""
foreach ($n in "sp3-none-pk-now","sp3-none-pk-now-cp0","sp3-d3-pk-now") { $t = (Get-Content "results\$n.out.txt" -Raw) -replace "\[ Prompt.*", ""; "{0}: {1}" -f $n, ($(if ($t -eq $ref) { "identical" } else { "DIFFERENT" })) }
