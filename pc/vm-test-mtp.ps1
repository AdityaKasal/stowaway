# 8 GB / 4 CPU / no swap VM; arguments are passed to vm/test-mtp.sh; restores .wslconfig afterwards
...
wsl --shutdown
wsl -d Ubuntu -u root -- bash /mnt/c/Users/FSociety/moe-router-study/vm/$(if ($env:MTP_SCRIPT) { $env:MTP_SCRIPT } else { "test-mtp.sh" }) $env:MTP_ARGS.Split(" ")
wsl --shutdown
