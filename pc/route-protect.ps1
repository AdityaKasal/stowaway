...
# base.kld exists from the first sweep; now: keep the model's own top 1 or 2 picks, swap only lower-ranked ones
foreach ($pr in "1", "2") { foreach ($b in "0.5", "1.0", "2.0") {
  $env:MOE_CACHE_PROTECT = $pr
  Run "protect$pr-bonus-$b" $b @("--kl-divergence-base", "$out\base.kld", "--kl-divergence")
} }
