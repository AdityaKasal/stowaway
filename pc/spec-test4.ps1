...
Run "sp4-d5-p05"  "2.6" ($draft + @("--spec-draft-n-max", "5", "--spec-draft-p-min", "0.5", "--no-warmup")) $true
Run "sp4-d8-p07"  "2.6" ($draft + @("--spec-draft-n-max", "8", "--spec-draft-p-min", "0.7", "--no-warmup")) $true
Run "sp4-d3-rep"  "2.6" ($draft + @("--spec-draft-n-max", "3", "--no-warmup")) $true
"`n--- same text?"
$ref = (Get-Content results\spec-none.out.txt -Raw) -replace "\[ Prompt.*", ""
foreach ($n in "sp4-d5-p05","sp4-d8-p07","sp4-d3-rep") { $t = (Get-Content "results\$n.out.txt" -Raw) -replace "\[ Prompt.*", ""; "{0}: {1}" -f $n, ($(if ($t -eq $ref) { "identical" } else { "DIFFERENT" })) }
