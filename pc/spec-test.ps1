...
Run "spec-none"  "3.2" @()
Run "spec-d1"    "2.6" ($draft + @("--spec-draft-n-max", "1"))
Run "spec-d2"    "2.6" ($draft + @("--spec-draft-n-max", "2"))
Run "spec-d3"    "2.6" ($draft + @("--spec-draft-n-max", "3"))
"`n--- same text?"
$ref = (Get-Content results\spec-none.out.txt -Raw) -replace "\[ Prompt.*", ""
foreach ($n in "spec-d1","spec-d2","spec-d3") { $t = (Get-Content "results\$n.out.txt" -Raw) -replace "\[ Prompt.*", ""; "{0}: {1}" -f $n, ($(if ($t -eq $ref) { "identical" } else { "DIFFERENT" })) }
