...
$prompt = "Explain in a short paragraph why the sky is blue."
...
Run "sp5-d8-p08"   "2.6" ($draft + @("--spec-draft-n-max", "8", "--spec-draft-p-min", "0.8", "--no-warmup")) $true
Run "sp5-d12-p08"  "2.6" ($draft + @("--spec-draft-n-max", "12", "--spec-draft-p-min", "0.8", "--no-warmup")) $true
Run "sp5-d16-p09"  "2.6" ($draft + @("--spec-draft-n-max", "16", "--spec-draft-p-min", "0.9", "--no-warmup")) $true
Run "sp5-d8-p07"   "2.6" ($draft + @("--spec-draft-n-max", "8", "--spec-draft-p-min", "0.7", "--no-warmup")) $true
"`n--- same text?"
$ref = (Get-Content results\spec-none.out.txt -Raw) -replace "\[ Prompt.*", ""
foreach ($n in "sp5-d8-p08","sp5-d12-p08","sp5-d16-p09","sp5-d8-p07") { $t = (Get-Content "results\$n.out.txt" -Raw) -replace "\[ Prompt.*", ""; "{0}: {1}" -f $n, ($(if ($t -eq $ref) { "identical" } else { "DIFFERENT" })) }
