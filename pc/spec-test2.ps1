...
Run "sp2-d3"       "2.6" ($draft + @("--spec-draft-n-max", "3")) $false
Run "sp2-d3-pk"    "2.6" ($draft + @("--spec-draft-n-max", "3")) $true
Run "sp2-d4-pk"    "2.6" ($draft + @("--spec-draft-n-max", "4")) $true
Run "sp2-d6-pk"    "2.6" ($draft + @("--spec-draft-n-max", "6")) $true
Run "sp2-none-pk"  "3.2" @() $true
"`n--- same text?"
$ref = (Get-Content results\spec-none.out.txt -Raw) -replace "\[ Prompt.*", ""
foreach ($n in "sp2-d3","sp2-d3-pk","sp2-d4-pk","sp2-d6-pk","sp2-none-pk") { $t = (Get-Content "results\$n.out.txt" -Raw) -replace "\[ Prompt.*", ""; "{0}: {1}" -f $n, ($(if ($t -eq $ref) { "identical" } else { "DIFFERENT" })) }
